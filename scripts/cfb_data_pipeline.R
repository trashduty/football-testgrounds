...
#
# Expects the following files already present in data/ (downloaded by the
# workflow's "Download historical data" step):
...
if (file.exists("data/CFB_GAMEDATA_2026_WEEK1.csv")) {
  gamedata_2026 <- read_csv("data/CFB_GAMEDATA_2026_WEEK1.csv",
                            show_col_types = FALSE)
  message(sprintf("  Loaded 2026 seed data: %d rows", nrow(gamedata_2026)))
}
...
current_week <- if (!is.null(weeks_with_data) && nrow(weeks_with_data) > 0) {
  max_completed <- weeks_with_data %>%
    filter(!is.na(games)) %>%
    pull(week) %>%
    max(na.rm = TRUE)
  if (is.infinite(max_completed)) 1L else as.integer(max_completed)
} else {
  1L
}
...
} else if (!is.null(gamedata_2026)) {
  master <- gamedata_2026
  message("  Falling back to CFB_GAMEDATA_2026_WEEK1.csv as game spine.")
} else {
...
# Select a sensible subset of columns for the search JSON to keep file size manageable
json_cols <- intersect(
  c(
    "game_id", "season", "week", "season_type",
    "home_team", "away_team", "home_conference", "away_conference",
